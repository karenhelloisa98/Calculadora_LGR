...
se movimentam no plano complexo $s$ à medida que o ganho do controlador ($K$) varia de zero a infinito.
...
g_num_str = st.sidebar.text_input("G(s) - Numerador", "1")
g_den_str = st.sidebar.text_input("G(s) - Denominador", "1 6 8")
h_num_str = st.sidebar.text_input("H(s) - Numerador", "1 1")
h_den_str = st.sidebar.text_input("H(s) - Denominador", "1 4 0")

st.sidebar.header("🎯 Teste de Ponto no LGR")
si_real = st.sidebar.number_input("Parte real do ponto de teste ($s_i$):", value=-1.00, format="%.2f")
si_imag = st.sidebar.number_input("Parte imaginária do ponto de teste ($s_i$):", value=1.00, format="%.2f")
tolerancia = st.sidebar.number_input("Tolerância (graus):", value=10.00, format="%.2f")
